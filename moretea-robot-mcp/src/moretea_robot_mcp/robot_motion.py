...
MAX_DURATION_S = 10.0  # hard cap to prevent runaway motion
...
class RobotMotionProvider:
    ...
    def move(
        ...
        safe_duration = min(max(0.0, float(duration_s)), MAX_DURATION_S)
        ...
        if pos_fn is not None:
            ...
            if initial_pos is not None:
                return self._move_distance_closed_loop(
                    dist, linear_x, tolerance_m, pos_fn, initial_pos
                )

        # Open-loop fallback (less accurate — used when odometry unavailable)
        duration_s = min(abs(dist) / safe_speed, MAX_DURATION_S)
    ...
    def _move_distance_closed_loop(
        ...
        # Timeout: open-loop estimate with 50% headroom, capped at MAX_DURATION_S
        open_loop_est = abs(target_m) / abs(linear_x)
        timeout_s = min(open_loop_est * 1.5 + 1.0, MAX_DURATION_S)
        ...
        if yaw_fn is not None:
            ...
            if initial_yaw is not None:
                return self._rotate_closed_loop(
                    target_deg, angular_z, tolerance_rad, yaw_fn, initial_yaw
                )

        # Open-loop fallback (less accurate — used when odometry unavailable)
        duration_s = min(abs(target_rad) / safe_speed, MAX_DURATION_S)
